...
_OUTPUT_CONSTRAINT = (
    ...
    "正文必须遵循 `standup-template.md` 的章节结构：标题、开场 Hook、建立世界观、深挖细节、"
    "脆弱时刻（可选）、疯狂升级、收尾 Call Back。\n"
    "严禁输出分析过程、主题说明、人设设计、核心观点、喜剧机制解释、爆点分析、meta说明、创作思路。"
)
...
class StandupFocusedArgs(BaseModel):
    """聚焦版脱口秀创作参数 Schema。"""
...
class StandupFocusedSkill(ComedySkill):
    """聚焦版脱口秀段子生成器。

    基于五感幽默系统、开放麦级别写法与增强模式输出段子正文。
    """

    task_type: str = "creative"
    name: str = "standup_focused"
    available_styles: ClassVar[list[str]] = ["日常观察", "自嘲", "社会讽刺", "职场", "黑色幽默", "吐槽"]
    description: str = (
        "创作聚焦版脱口秀段子。输入主题、态度、偏见、情绪、时长，"
        "基于五感幽默系统、开放麦级别写法与增强模式输出段子正文。"
    )
    args_schema: type[BaseModel] = StandupFocusedArgs
    ...
    def _run(
        ...
        docs = self._retrieve_knowledge(topic, user_id, kind="standup", style="日常观察")
